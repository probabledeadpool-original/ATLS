...
const themeOptions: { value: Theme; cOption: string; icon: React.ReactNode }[] = [
  {
    ...
    icon: (
      <svg
        className="w-4 h-4 text-current relative z-10"
...
export function ThemeSwitcher({
  ...
  return (
    <fieldset
      className="relative flex items-center justify-between p-1 rounded-full border border-white/20 dark:border-white/10 shadow-[0_8px_16px_rgba(0,0,0,0.1),inset_0_1px_1px_rgba(255,255,255,0.2)] bg-[#111111]/[0.08] dark:bg-black/[0.2] backdrop-blur-[20px]"
      style={{ width: "128px" }}
    >
      <legend className="sr-only">Choose theme</legend>

      {themeOptions.map((option) => (
        <label key={option.value} className="relative flex-1 flex items-center justify-center cursor-pointer py-1.5 rounded-full transition-colors z-10 hover:text-foreground">
          <input
          ...
          {activeValue === option.value && (
            <motion.div 
              ...
              className="absolute inset-0 bg-foreground shadow-[0_1px_4px_rgba(0,0,0,0.1)] rounded-full z-0" 
