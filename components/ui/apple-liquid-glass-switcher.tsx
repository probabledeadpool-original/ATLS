...
const themeOptions: { value: Theme; cOption: string; icon: React.ReactNode }[] = [
  {
    ...
    icon: (
      <svg
        className="w-5 h-5 text-current relative z-10"
...
export function ThemeSwitcher({
  ...
  return (
    <fieldset
      className="relative flex items-center justify-between p-1 rounded-full border border-white/20 dark:border-white/10 shadow-[0_8px_16px_rgba(0,0,0,0.1),inset_0_1px_1px_rgba(255,255,255,0.2)] bg-black/[0.08] dark:bg-black/[0.2] backdrop-blur-[24px]"
      style={{ width: "160px" }}
      data-previous={
    ...
    >
      ...
      {themeOptions.map((option) => (
        <label key={option.value} className="relative flex-1 flex items-center justify-center cursor-pointer p-2 rounded-full transition-colors z-10 hover:text-foreground">
          <input
          ...
          />
          <div className="text-muted-foreground peer-checked:text-background transition-colors z-20">
            {option.icon}
          </div>
          {activeValue === option.value && (
            <div className="absolute inset-0 bg-foreground shadow-[0_1px_4px_rgba(0,0,0,0.1)] rounded-full pointer-events-none z-0" />
          )}
        </label>
      ))}
